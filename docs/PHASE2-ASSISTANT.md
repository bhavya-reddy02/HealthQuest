# HealthQuest — Phase 2: the AI Health Coach (Claude + RAG)

Phase 2 adds a personal health assistant to HealthQuest. It uses **Claude** for the
...
In the **`healthquest-backend`** folder, create a file named exactly **`.env`** with one line:
...
- On startup the small embedding model downloads once (~130 MB) and the knowledge
  ...
  **`[healthquest] knowledge index ready — indexed N chunks`**.
...
Start the frontend as usual (`cd healthquest-frontend && npm run dev`), open
