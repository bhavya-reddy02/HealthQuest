# HealthQuest API — Phase 1 Backend

The database layer for the HealthQuest health platform:
...
**Start everything** (from inside this `healthquest-backend` folder):
...
- Postgres:       localhost:5432  (user `healthquest`, password `healthquest_pw`, db `healthquest`)
...
CREATE ROLE healthquest WITH LOGIN PASSWORD 'healthquest_pw';
CREATE DATABASE healthquest OWNER healthquest;
```
...
## 7. Connecting the HealthQuest frontend

The current `HealthQuest.jsx` saves to the browser's storage. To use this API instead, replace
