...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        n = ensure_index()
        print(f"[healthquest] knowledge index ready" + (f" — indexed {n} chunks" if n else " (already built)"))
    except Exception as e:  # pragma: no cover
        print(f"[healthquest] WARNING: knowledge index not built yet: {e}")

    yield


app = FastAPI(title="HealthQuest API", version="1.0.0", lifespan=lifespan)
...
@app.get("/", tags=["health"])
def health():
    return {"status": "ok", "service": "healthquest-api", "version": "1.0.0"}
