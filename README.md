...
**Health** turns healthy habits into a game. Users complete **daily quests**, earn **XP**,
...
**2. Add an AI key** — create `backend/.env` (free option shown):
```
LLM_PROVIDER=gemini
...
```
HealthQuest/
├── backend/     # FastAPI · Postgres · Redis · LangChain + ChromaDB
│   └── app/
...
`backend/app/rag/knowledge/hospital-info.md`, then
...
| Symptom | Fix |
|---------|-----|
| `no configuration file provided` | `cd backend` before `docker compose ...` |
...
<div align="center">
Built as an MSc dissertation project. HealthQuest provides general wellbeing information, not medical advice.
</div>